...
{
    uint16_t curr_x = joystick_read_x();
    uint16_t curr_y = joystick_read_y();

    if(curr_x > JOYSTICK_THRESH_X_LEFT_2P7V) {
        if(curr_y < JOYSTICK_THRESH_Y_DOWN_0P6V) {
            return JOYSTICK_POS_LOWER_LEFT;
        }
        if(curr_y > JOYSTICK_THRESH_Y_UP_2P7V) {
            return JOYSTICK_POS_UPPER_LEFT;
        }
        return JOYSTICK_POS_LEFT;
    }

    else if(curr_x < JOYSTICK_THRESH_X_RIGHT_0P6V) {
        if(curr_y < JOYSTICK_THRESH_Y_DOWN_0P6V) {
