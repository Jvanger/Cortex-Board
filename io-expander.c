...
static void io_expander_write_reg(uint8_t reg, uint8_t value)
{
	...
	cyhal_i2c_master_write(&i2c_monarch_obj, TCA9534_SUBORDINATE_ADDR, write_data, 2, 0, true);
...
static uint8_t io_expander_read_reg(uint8_t reg)
{
	...
	/* ADD CODE */
	/* Use cyhal_i2c_master_write to write the required data to the device. */
	cyhal_i2c_master_write(&i2c_monarch_obj, TCA9534_SUBORDINATE_ADDR, write_data, 1, 0, true);
	
	CY_ASSERT(rslt == CY_RSLT_SUCCESS);

	/* ADD CODE */
	/* Use cyhal_i2c_master_read to read the required data from the device. */
	cyhal_i2c_master_read(&i2c_monarch_obj, TCA9534_SUBORDINATE_ADDR, read_buffer, 1, 0, true);
